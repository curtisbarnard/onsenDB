L.GeoCSV = L.GeoJSON.extend({
  ...
  _propertiesNames: [],
  _re: { spaces: /\s+/g, notFloats: /[^0-9., -]/g, notWord: /[^\w]/g },
  ...
  _checkOptions: function (opts) {
    if (
      this.options.activeWKT &&
      !opts.WKTTitle &&
      !this.options.firstLineTitles
    )
      this._debug('WARN: WKTTitle not defined', false);
    if (this.options.activeWKT && this.options.decimalSeparator != '.')
      this._debug('WARN: In WKT mode decimalSeparator need to be a dot', false);
    if (this.options.firstLineTitles && opts.titles)
      this._debug('WARN: With firstLineTitles titles array not apply', false);
    if (this.options.fieldSeparator.length != 1)
      this._debug('WARN: fieldSeparator need to be a single character', false);
    if (this.options.lineSeparator.length != 1)
      this._debug('WARN: lineSeparator need to be a single character', false);
  },
  ...
  addData: function (data) {
    if (typeof data === 'string') {
      ...
      for (var i = 0; i < this.options.titles.length; i++) {
        var prop = this.options.titles[i]
          .toLowerCase()
          .replace(this._re.spaces, '_')
          .replace(this._re.notWord, '');
  ...
  _csv2array: function (str) {
    ...
    for (var row = (col = c = 0); c < str.length; c++) {
      ...
      if (cc == this.options.lineCommentSeparator && !quote) {
        comment = true;
        continue;
      }
      if (cc == '"' && quote && nc == '"') {
      ...
      if (cc == this.options.fieldSeparator && !quote) {
        ++col;
        continue;
      }
      if (cc == '\r' && nc == this.options.lineSeparator && !quote) {
        ++row;
        col = 0;
        ++c;
        comment = false;
      ...
      if (cc == this.options.lineSeparator && !quote) {
        ++row;
        col = 0;
        comment = false;
        continue;
  ...
  _parseWKTFeature: function (wkt) {
    ...
    for (var g = 0; g < groups.length; g++) {
      var coordinates_group = groups[g]
        .replace(this._re.notFloats, '')
        .trim()
        .split(',');
      coordinates[g] = [];
  ...
  _csv2json: function (csv) {
    ...
    for (var num_linea = 0; num_linea < csv.length; num_linea++) {
      ...
      if (campos.length == 1 && campos[0].trim() == '') {
        this._debug(
          'INFO: Blank or commented at line ' + (num_linea + incr),
          campos
        );
        continue;
      }
      if (campos.length != titulos.length) {
        this._debug(
          "ERR: Fields and titles don't match at line " +
            (num_linea + incr) +
            ':',
          campos
        );
      ...
      if (this.options.activeWKT) {
        feature = this._parseWKTFeature(
          campos[titulos.indexOf(this.options.WKTTitle)]
        );
      } else {
        var lng = parseFloat(
            campos[titulos.indexOf(this.options.longitudeTitle)].replace(
              this.options.decimalSeparator,
              '.'
            )
          ),
          lat = parseFloat(
            campos[titulos.indexOf(this.options.latitudeTitle)].replace(
              this.options.decimalSeparator,
              '.'
            )
          );
        if (lng < 180 && lng > -180 && lat < 90 && lat > -90) {
          ...
          this._debug(
            'ERR: Invalid LatLon pair at line ' + (num_linea + incr),
            false
          );
        }
      }
      if (feature.geometry && feature.geometry.type) {
        for (var i = 0; i < titulos.length; i++) {
          if (
            (this.options.activeWKT && titulos[i] != this.options.WKTTitle) ||
            (!this.options.activeWKT &&
              titulos[i] != this.options.latitudeTitle &&
              titulos[i] != this.options.longitudeTitle)
          ) {
        ...
        this._debug(
          'ERR: Invalid feature at line ' + (num_linea + incr) + ':',
          campos
        );
      }
    }
